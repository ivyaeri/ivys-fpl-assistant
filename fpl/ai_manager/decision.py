...
from fpl.ai_manager.core import pick_starting_xi, simulate_week_points
...
def decide_with_ai(
    ...
    squad_txt = (
        market_df[market_df["id"].isin(squad_ids)][
            [
                "id",
                "web_name",
                "team_short",
    ...
    system = (
        "You are an autonomous FPL manager. Return STRICT JSON only with required keys."
    )
    user = f"""
Gameweek: {gw_now}
...
"""
    ...
    dec = _safe_json(raw) or {
        "chip": "NONE",
        "made": False,
        "out_id": None,
        "in_id": None,
        "captain_id": None,
        "reason": "Parse failed; hold.",
    }
    if dec.get("chip") not in {"NONE", "TC", "BB", "FH", "WC"}:
        dec["chip"] = "NONE"
    if not isinstance(dec.get("made"), bool):
        dec["made"] = False
    return dec


def apply_transfer_if_legal(
    state: dict, market: pd.DataFrame, decision: dict
) -> tuple[bool, str]:
    if not decision.get("made"):
        return False, "Hold chosen."
    if state["free_transfers"] < 1:
        return False, "No free transfers."
    ...
    if out_id not in squad_ids or in_id in squad_ids:
    ...
    if out_row.empty or in_row.empty:
        return False, "Player not in market."
    if out_row.iloc[0]["pos"] != in_row.iloc[0]["pos"]:
        return False, "Not like-for-like."
    delta = float(in_row.iloc[0]["price"]) - float(out_row.iloc[0]["price"])
    if delta > state["bank"] + 1e-6:
        return False, "Over budget."
    temp_ids = [sid for sid in squad_ids if sid != int(out_id)] + [int(in_id)]
    temp_sq = market[market["id"].isin(temp_ids)]
    if temp_sq.groupby("team_short")["id"].count().max() > 3:
    ...
    for i, p in enumerate(state["squad"]):
        if p["id"] == int(out_id):
            state["squad"][i] = {
                "id": int(in_id),
                "buy_price": float(in_row.iloc[0]["price"]),
            }
            break
...
def run_ai_auto_until_current(
    ...
    # build scored market for XI/captain picking; AI still decides transfers/chips
    market = players_df.copy()
    from fpl.ai_manager.core import build_scored_market as _score

    market = _score(market, n_fixt=3)
    ...
    for gw in range(state["last_gw_processed"] + 1, gw_now + 1):
        ...
        pts = simulate_week_points(
            xi_df, captain_id, gw, chip=chip, bench_ids=bench_ids
        )
        ...
        state["log"].append(
            {
                "gw": int(gw),
                "transfer": {"out": ai_dec.get("out_id"), "in": ai_dec.get("in_id")}
                if made
                else None,
                "made": bool(made),
